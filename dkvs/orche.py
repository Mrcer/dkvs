...
class OrcheMain:
    ...
    def add_store(self, store_id: UUID, addr: str, state):
        ...
        with self.store_states_lock:
            ...
            logger.info(
                f"[Orche] Store {str(store_id)[:5]} added at {addr}, state={state}"
            )
    ...
    def remove_store(self, store_id: UUID):
        ...
        with self.store_states_lock:
            if self.store_states.remove_node(store_id):
                ...
                logger.warning(
                    f"[Orche] Attempted to remove unknown Store ID: {str(store_id)[:5]}"
                )
        ...
        while self.running:
            ...
            with self.store_states_lock:
                ...
                for store_id in stale_stores:
                    logger.warning(
                        f"[Orche] Store {str(store_id)[:5]} is stale, removing..."
                    )
    ...
    def task_update_ring(self):
        ...
        while self.running and self.store_states_changed:
            ...
            with self.store_states_lock:
                infos = [
                    n.to_dict() for n in self.store_states.get_all_nodes().values()
                ]
    ...
    def set_store_state(self, store_id: str, state: int) -> bool:
        ...
        with self.orche.store_states_lock:
            ...
            if store_info:
                ...
                logger.info(
                    f"[Orche] Store {store_id[:5]} is now in state {store_info.state.name}"
                )
                return True
            else:
    ...
    def heartbeat(self, store_id: str) -> bool:
        ...
        with self.orche.store_states_lock:
            ...
            if store_info:
                ...
                logger.warning(
                    f"[Orche] Heartbeat from unknown Store ID: {store_id[:5]}"
                )
                return False
