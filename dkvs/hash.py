...
class ConsistentHash:
    ...
    def get_next_node(
        self, key: str, selector: Callable[[StoreInfo], bool]
    ) -> StoreInfo | None:
        """获取key对应的下一个节点"""
        if not self.ring:
            return None
    ...
    def get_node_pair_in_state(
        self, key: str, selector: Callable[[StoreInfo], bool]
    ) -> tuple[StoreInfo, StoreInfo] | None:
        """获取key前后的指定状态节点"""
    ...
    def get_primary_node(self, key: str) -> StoreInfo | None:
        """获取key对应的主节点"""
        pnode = self.get_next_node(
            key, lambda node: node.state is not StoreState.INITIALIZING
        )
        return pnode
    ...
    def is_in_range(self, h: int, pnode: UUID) -> bool:
        ...
        prev_node = self.get_prev_node(
            str(pnode), lambda n: n.state is not StoreState.INITIALIZING
        )
        if not prev_node:
            return False
