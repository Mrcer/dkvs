...
class StoreMain:
    ...
    def get(self, key: str) -> tuple[Literal['NOT_FOUND', 'WRONG_NODE', 'SUCCESS', 'TEMPORARY_ERROR'], str]:
        """从本地存储获取值，包括主存储和备份存储"""
        # 从任意StoreDB获取值
        with self.store_states_lock:
        ...
        while True:
            ...
            if self.store_states_lock.locked():
                ...
                if self.lock_count > 1:
                    logging.warning(f"[Store-{str(self.store_id)[:5]}] Lock detected for {self.lock_count} times, maybe dead lock")
            else:
                self.lock_count = 0
...
class StoreRPC:
    ...
    def client_get(self, key: str) -> tuple[Literal['NOT_FOUND', 'WRONG_NODE', 'SUCCESS', 'TEMPORARY_ERROR'], str]:
        """获取值"""
    ...
    def fetch_range_data(self, _hash_from: str, _hash_to: str) -> dict | None:
        ...
        with self.store.store_states_lock:
            ...
            is_from_in_range = store_states.is_in_range(hash_from + 1, self.store.store_id)
